...
const retryLink = new RetryLink({
  ...
  attempts: {
    max: 5,
    retryIf: (error, operation) => Boolean(error) && ![400, 401, 403, 404].includes(error.statusCode)
  }
});
...
function getConfig(options) {
  ...
  invariant(config.publicAPIKey && config.publicAPIKey.length > 0, `@builder.io/gatsby requires a public API Key`);
  ...
  const graphqlOptions = {
    ...
    createLink: async ({
      ...
      return ApolloLink.from([retryLink, createDataloaderLink({
        uri: url,
        fetch: nodeFetch
      })]);
    }
